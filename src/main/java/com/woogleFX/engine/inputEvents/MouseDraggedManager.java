...
public class MouseDraggedManager {
    ...
    public static void eventMouseDragged(MouseEvent event) {
        ...
        MousePressedManager.wasDragged = true;
        ...
        if (SelectionManager.getMode() == SelectionManager.GEOMETRY) {
            if (SplineManager.getSelected1() != -1 || SplineManager.getSelected2() != -1) {
                handleGeometryMode(event, gameRelativeMouseX, gameRelativeMouseY);
            }
        }

        if (level.getSelectedComponents().length != 0 && SelectionManager.getDragSettings() != null) {

            Point2D mousePos = new Point2D(gameRelativeMouseX, gameRelativeMouseY);
            ...
            switch (SelectionManager.getDragSettings().getType()) {
                case DragSettings.MOVE -> ObjectDrag.dragFromMouse(mousePos, SelectionManager.getDragSettings());
                case DragSettings.RESIZE -> ObjectResize.resizeFromMouse(mousePos, SelectionManager.getDragSettings());
                case DragSettings.ROTATE -> ObjectRotate.rotateFromMouse(mousePos, SelectionManager.getDragSettings());
                case DragSettings.SETANCHOR -> ObjectSetAnchor.setAnchor(mousePos, SelectionManager.getDragSettings());
            }

            FXPropertiesView.getPropertiesView().refresh();
        }

        if (event.getButton() == MouseButton.SECONDARY && SelectionManager.getMouseStartX() != 0 && SelectionManager.getMouseStartY() != 0) {
    ...
    private static void handleGeometryMode(MouseEvent event, double gameRelativeMouseX, double gameRelativeMouseY) {
