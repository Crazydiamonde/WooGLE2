...
public class FileManager {
    ...
    public static void openFailedImage() {
        try {
    ...
    public static void readWOGdirs() {
        try {
    ...
    public static WorldLevel openLevel(String levelName, double version) {
        try {
            scene = new ArrayList<>();
            level = new ArrayList<>();
    ...
    public static ArrayList<EditorObject> getScene(String levelName, double version) {
        try {
            scene = new ArrayList<>();
            SAXParserFactory factory = SAXParserFactory.newInstance();
            SAXParser saxParser = factory.newSAXParser();
    ...
    public static _Ball openBall(String ballName, double version) {
        try {
    ...
    public static ArrayList<EditorObject> openResources(double version) {
        try {
            commonBallResrcData = new ArrayList<>();
            SAXParserFactory factory = SAXParserFactory.newInstance();
            SAXParser saxParser = factory.newSAXParser();
    ...
    public static ArrayList<EditorObject> openParticles(double version) {
        try {
            commonBallData = new ArrayList<>();
            commonBallResrcData = new ArrayList<>();
            SAXParserFactory factory = SAXParserFactory.newInstance();
    ...
    public static ArrayList<EditorObject> openText(double version) {
            try {
            commonBallData = new ArrayList<>();
            SAXParserFactory factory = SAXParserFactory.newInstance();
            SAXParser saxParser = factory.newSAXParser();
    ...
    public static void main(String[] args) {
        try {
            ...
            ArrayList<ArrayList<String>> attributeNames = new ArrayList<>();

            SAXParserFactory factory = SAXParserFactory.newInstance();
            SAXParser saxParser = factory.newSAXParser();
            for (File file : new File(FileManager.getNewWOGdir() + "\\res\\levels").listFiles()) {
                for (File inFile : file.listFiles()) {
                    if (inFile.getName().endsWith(".scene") || inFile.getName().endsWith(".level") || inFile.getName().endsWith(".resrc")) {
                        saxParser.parse(inFile, new DefaultHandler() {

                            @Override
                            public void startElement(String uri, String localName, String qName, Attributes attributes) {
                                if (!names.contains(qName)) {
                                    names.add(qName);
                                    ArrayList<String> these = new ArrayList<>();
                                    for (int j = 0; j < attributes.getLength(); j++) {
                                        these.add(attributes.getQName(j));
                                    }
                                    attributeNames.add(these);
                                } else {
                                    for (String attributeName : attributeNames.get(names.indexOf(qName)).toArray(new String[0])) {
                                        boolean contains = false;
                                        for (int j = 0; j < attributes.getLength(); j++) {
                                            if(attributes.getQName(j).equals(attributeName)) {
                                                contains = true;
                                            }
                                        }
                                        if (!contains) {
                                            attributeNames.get(names.indexOf(qName)).remove(attributeName);
                                        }
                                    }
                                }
            ...
            for (int i = 0; i < names.size(); i++) {
                ...
                for (String attribute : attributeNames.get(i)) {
                    System.out.print(attribute);
                    if (!attribute.equals(attributeNames.get(i).get(attributeNames.get(i).size() - 1))) {
                        System.out.print(", ");
                    }
                }
                System.out.println("}");
